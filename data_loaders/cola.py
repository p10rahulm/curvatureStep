...
# Load your dataset
train_iter, test_iter = CoLA(split=('train', 'test'))
...
def collate_batch(batch, vocab=vocab):
    ...
    for (_, label, text) in batch:
        label_list.append(label_pipeline(label))
        processed_text = torch.tensor(text_pipeline(text, vocab), dtype=torch.int64)
