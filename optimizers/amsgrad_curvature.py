...
class AMSGradCurvature(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0, epsilon=1e-8, r_max=10.0):
    ...
    def step(self, closure=None):
        ...
        # Update with curvature and AMSGrad
        param_idx = 0
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is None:
                ...
                state['max_exp_avg_sq'].copy_(saved['max_exp_avg_sq'])
                ...
                if weight_decay != 0:
                    grad_with_curvature.add_(orig_data, alpha=weight_decay)
                
                # AMSGrad update with curvature
                ...
                exp_avg.mul_(beta1).add_(grad_with_curvature, alpha=1 - beta1)
                ...
                # Update maximum of second moment estimate
                ...
                # Compute denominator using max of second moments
