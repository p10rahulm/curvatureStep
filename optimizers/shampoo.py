...
def _matrix_power(matrix: torch.Tensor, power: float) -> torch.Tensor:
    ...
    matrix = matrix.to(torch.float64).cpu()  # Move to CPU for SVD
    u, s, v = torch.svd(matrix)
    return (u @ s.pow_(power).diag() @ v.t()).to(device)

class Shampoo(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-2, weight_decay=0, momentum=0.1, rho=0.9, update_freq=10, grad_clip=1.0):
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, momentum=momentum, rho=rho, update_freq=update_freq, grad_clip=grad_clip)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    state['step'] = 0
                    state['momentum_buffer'] = torch.zeros_like(p.data, device=device)
                    state['preconds'] = [eps * torch.eye(dim, device=device) for dim in grad.size()]
                    state['inv_preconds'] = [torch.zeros(dim, dim, device=device) for dim in grad.size()]
                ...
                for dim_id, dim in enumerate(grad.size()):
                    precond = state['preconds'][dim_id].to(device)
                    inv_precond = state['inv_preconds'][dim_id].to(device)
                    ...
                    if state["step"] % update_freq == 0:
                        try:
                            if torch.isnan(precond).any() or torch.isinf(precond).any():
                                raise ValueError("Preconditioner matrix contains NaNs or Infs.")
                            inv_precond.copy_(_matrix_power(precond + eps * torch.eye(precond.size(0), device=device), -1 / grad.ndimension()))
                        except torch.linalg.LinAlgError as e:
                            print(f"SVD failed for preconditioner matrix at step {state['step']}. Error: {e}")
                            continue
