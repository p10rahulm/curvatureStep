...
class NAdamCurvature(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, epsilon=1e-8, r_max=10.0):
        defaults = dict(lr=lr, betas=betas, eps=eps, epsilon=epsilon)
        self.r_max = r_max
        super(NAdamCurvature, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        # Update with curvature and NAdam
        param_idx = 0
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is None:
