# experiments/food101_training_runs.py
...
for optimizer_class, default_params in optimizers:
    print(f"\nRunning Food101 training with Optimizer = {str(optimizer_class.__name__)}")
    params = default_params.copy()

    # Set device to GPU
    device = torch.device('cuda:2' if torch.cuda.is_available() else 'cpu')

    dataset_loader = load_food101
...
write_to_file('outputs/food101_training_logs.csv', results)
